...
const FieldTypeEnum = z.enum(['TEXT', 'TEXTAREA', 'EMAIL', 'NUMBER', 'DROPDOWN', 'CHECKBOX', 'RADIO', 'FILE']);
...
export const addFormFieldSchema = z.object({
  body: z
    .object({
      ...
      name: z
        .string()
        .min(1, { message: 'Field internal name is required' })
    ...
    .refine(
      (data) => {
        // If type requires options (dropdown, radio, checkbox), options should exist
        if (
          ['DROPDOWN', 'RADIO', 'CHECKBOX'].includes(data.type) &&
